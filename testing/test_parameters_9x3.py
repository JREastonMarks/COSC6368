...
# Setup
vector_size = 9
label_input = np.array([1, 0, 0, 0, 1, 0, 0, 0, 1], np.float32).reshape((vector_size))
label_goal = np.array([1, 0, 0], np.float32).reshape((3))

results = open("results_9x3.tsv", "x")
...
for omega_size in range(1, 90, 1):
    ...
    for excitation_level in range(1, 9, 1):
        ...
        for test_number in range(100):
            label_encoder = CorticalMiniColumn(layer_sizes=[9,9,3], omega=omega_size, excitation=excitation_level)
